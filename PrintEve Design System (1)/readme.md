...
- `digital-card/` — the NFC digital business card **receiver view** (the page opened by tapping a card / scanning its QR). Professional card-on-canvas layout (LinkedIn profile pattern + Linear restraint), mobile-first 390×844 up to a desktop two-column layout with a sticky QR side rail. Two data-driven demo personas (business / individual), dark mode, Save Contact (real vCard), copy-on-tap details, WhatsApp-enquiry services, lead-capture sheet, `?embed=1` chrome-less mode. See its README and `d:\NFC Card\nfc-card-information-architecture.md`.
- `studio/` — the **Card Studio owner dashboard**: Overview stats, searchable Leads inbox with CSV export, Analytics (views chart, tap breakdown, cities), My Card editor with live phone-frame preview of the receiver view, and Share & Order (QR kit, email signature, printed-NFC-card cross-sell from ₹499). Sidebar collapses to a bottom nav on mobile. See its README.
